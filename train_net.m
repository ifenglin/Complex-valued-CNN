...
% use sample_loader
[city, field, forest, grass, street] = very_simple_data_loader(size, num_channels, num*epochs);
% create labels
...
% initial variables
data = zeros(size, size, num_channels, num*num_labels);
...
[test_errors(1,:,:), test_loss(1,:), test_est_labels(1,:), ...
    test_correctness_rate(1), test_output_data(1, :, :)] = f_test_net(myNet, size, num_channels, test_num);
...
for epoch = 1:epochs
    ...
    for i = 1:num
        for j = 1:num_labels
            data(:,:,:,(i-1)*num_labels+j) = inputs{j}(:,:,:,i+(epoch-1)*num);
        end
    end       
    ...
    [test_errors(epoch+1,:,:), test_loss(epoch+1,:), test_est_labels(epoch+1, :),...
        ...
        f_test_net(myNet, size, num_channels, test_num);
end
